...
def run_gentle(audio_path, transcript_path):
	"""Uses Gentle's API to perform the forced Alignment"""
	...
	try:
		print("Sending files to Gentle API...")
...
def sing_along_app():
	...
	def start_lyrics(audio_path, lyrics_path):
		...
		if lyrics:
			...
			with open(lyrics_file, "w") as f:
				f.write(lyrics)
			gentle_output = run_gentle(converted_audio, lyrics_file)
			if gentle_output:
	...
	def select_files():
		...
		lyrics_path = filedialog.askopenfilename(
			title="Select Lyrics File (Optional)",
			initialdir="/app/files",
			filetypes=[
				("Text Files", "*.txt"),
				("All Files", "*")
