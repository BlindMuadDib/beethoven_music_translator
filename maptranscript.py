...
def calculate_midpoints(synchronized_transcript):
    """Calculate midpoints for transitioning between lines."""
    try:
        midpoints = []

        for index in range(len(synchronized_transcript) - 1):
            current_line = synchronized_transcript[index]
            next_line = synchronized_transcript[index + 1]

            # Find the last aligned word in the current line
            last_word_end_time = current_line[-1][1]

            # Find the first aligned word in the next line
            first_word_start_time = next_line[0][0]

            # Calculate the midpoints
            midpoints.append((last_word_end_time + first_word_start_time) / 2)
            print(f"[DEBUG] Midpoint calculated: {midpoints[-1]}")

        return midpoints

    except Exception as e:
        print(f"[ERROR] Error calculating midpoints: {e}")
        return []
