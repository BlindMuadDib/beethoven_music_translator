...
def test_translate_endpoint_missing_lyrics(client: FlaskClient, mock_rq_components, mock_file_validation):
    """
    Test /translate endpoint with missing lyrics file
    should allow translation to accomodate instrumental music
    """
    with patch('musictranslator.main.is_session_valid', return_value=True):
        audio_file_path = "data/audio/BloodCalcification-SkinDeep.wav"
        audio_data = load_test_file(audio_file_path)
...
def test_download_mtr_endpoint_success(
    client: FlaskClient,
    mock_rq_components: dict,
    mock_uuid_generator: dict
):
    """Test the /api/download-mtr/<job_id> endpoint."""
    job_id = mock_uuid_generator['test_job_id']
    mock_job = mock_rq_components['job']
    ...
    with patch('musictranslator.main.is_session_valid', return_value=True):
        ...
        mock_job.result = {
            ...
            "harmonic_analysis": {"static_results_url": "file.json"},
            "mapped_result": [],
            "drum_analysis": {}
        }

        # Mock file handling using builtins.open and os.path.exists
        with patch('builtins.open', create=True) as mock_open, \
             patch('os.path.exists', return_value=True):

            # Configure the mock file to return bytes when read() is called
            mock_file_handle = mock_open.return_value.__enter__.return_value
            mock_file_handle.read.return_value = b'{"mock": "data"}'

            client.set_cookie('session', 'valid')
            response = client.get(f'/api/download-mtr/{job_id}')

            assert response.status_code == 200
            assert response.headers['Content-Type'] == 'application/zip'
            assert 'Song.mtr' in response.headers['Content-Disposition']

def test_download_mtr_endpoint_unauthorized(client: FlaskClient):
    """Test /api/download-mtr/<job_id> without auth."""
    with patch('musictranslator.main.is_session_valid', return_value=False):
        response = client.get('/api/download-mtr/some-id')
        assert response.status_code == 401

# --- /library endpoint tests ---

def test_get_library_success_sorted(client: FlaskClient):
    """Test that GET /api/library returns CC songs sorted by Artist then Title."""
    
    mock_metadata = [
        {
            "artist": "Depressed",
            "title": "Richard Omelette",
            "filename_prefix": "Depressed-RichardOmelette",
            "license": "Creative Commons"
        },
        {
            "artist": "Blood Calcification",
            "title": "Skin Deep",
            "filename_prefix": "BloodCalcification-SkinDeep",
            "license": "Creative Commons"
        },
        {
            "artist": "Blood Calcification",
            "title": "No More",
            "filename_prefix": "BloodCalcification-NoMore",
            "license": "Creative Commons"
        },
        {
            "artist": "Evil Corp",
            "title": "Copyrighted Song",
            "filename_prefix": "EvilCorp-Copyrighted",
    ...
    with patch('os.path.exists', return_value=True), \
        ...
        # Verify sorting: Blood Calcification before Depressed, No More before Skin Deep
        assert data[0]["artist"] == "Blood Calcification"
        assert data[0]["title"] == "No More"
        assert data[0]["mtr_url"] == "/api/library/file/BloodCalcification-NoMore.mtr"
        assert data[0]["audio_url"] == "/api/files/BloodCalcification-NoMore.wav"

        assert data[1]["artist"] == "Blood Calcification"
        assert data[1]["title"] == "Skin Deep"
        
        assert data[2]["artist"] == "Depressed"
        assert data[2]["title"] == "Richard Omelette"

        # Verify Evil Corp song is NOT included
        assert not any(song["artist"] == "Evil Corp" for song in data)

def test_get_library_file(client: FlaskClient):
    """Test that GET /api/library/file/<filename> serves the .mtr file."""
    with patch('musictranslator.main.send_from_directory') as mock_send:
        mock_send.return_value = b"archive content"
        
        response = client.get('/api/library/file/BloodCalcification-NoMore.mtr')
        assert response.status_code == 200
        assert b"archive content" in response.data
        mock_send.assert_called_once_with('/shared-data/translated', 'BloodCalcification-NoMore.mtr', as_attachment=False)

def test_get_library_no_metadata(client: FlaskClient):
    """Test GET /api/library when metadata.json is missing."""
