...
class TestDrumMLA(unittest.TestCase):
    ...
    def test_vectorized_batch_classification(self):
        """
        Test the new classify_drum_events_batch method.
        It should yield consistent results with the iterative method but use
        vectorization.
        """
        if len(self.real_known_samples) < 5:
            self.skipTest("Not enough real samples for batch test.")

        # Create a batch of hits (just reuse some real samples)
        batch_hits = [
            self.mla._prepare_features(s) for s in self.real_known_samples[:5]
            if s.get('drum_category') and s.get('drum_type')
        ]
        # Add required keys that might be missing after _prepare_features
        for i, hit in enumerate(batch_hits):
            hit['onset_time'] = 0.1 * i
            hit['duration'] = 0.5

        # 1. Run Old Iterative Method (Ground Truth)
        # Force sequential to ensure we are testing the logic, not the pool
        start_iter = self.real_known_samples[:5] # Use raw for iterative as it expects raw-ish
        iterative_results = self.mla.classify_drum_events(start_iter, executor=None)

        # 2. Run New Vectorized Method
        # The vectorized method should attempt to list the feature dicts
        vectorized_results = self.mla.classify_drum_events_batch(start_iter)

        # 3. Compare
        self.assertEqual(len(iterative_results), len(vectorized_results))

        for it_res, vec_res in zip(iterative_results, vectorized_results):
            self.assertEqual(it_res['drum_category'], vec_res['drum_category'])
            self.assertEqual(it_res['drum_type'], vec_res['drum_type'])
            # Floating point tolerance
            self.assertAlmostEqual(it_res['category_confidence'],
                                   vec_res['category_confidence'], places=5)
