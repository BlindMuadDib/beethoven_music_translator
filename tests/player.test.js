...
describe('Player Facade Integration', () => {
    ...
    describe('Instrumental Mode (No Lyrics)', () => {
        test('should NOT initialize LyricTracker and should HIDE canvas if mapped_result is empty', () => {
            // Arrange: Create data with empty lyrics
            const instrumentalData = {
                ...mockResultData,
                mapped_result: [] // Empty array implies no lyrics
            };
            const lyricCanvas = document.getElementById('lyric-canvas');

            // Act
            initPlayer(instrumentalData, jest.fn(), mockedDependencies);

            // Assert
            // 1. LyricTracker should NOT be instantiated
            expect(mockLyricTracker).not.toHaveBeenCalled();

            // 2. The canvas should be hidden to avoid taking up empty space
            expect(lyricCanvas.style.display).toBe('none');

            // 3. Other trackers should still initialize
            expect(mockHarmonicVisualizer).toHaveBeenCalled();
            expect(mockDrumTracker).toHaveBeenCalled();
        });

        describe('renderLoop', () => {
            test('renderLoop should not crash if lyricTracker is null', async () => {
                // Arrange
                const instrumentalData = { ...mockResultData, mapped_result: [] };
                initPlayer(instrumentalData, jest.fn(), mockedDependencies);
                const audioPlayer = document.getElementById('audio-player');

                // Act: Trigger play to start render loop
                ...
                // Verify other trackers still updated
                expect(mockHarmonicVisualizerInstance.update).toHaveBeenCalledWith(1.0);
            });

            test('renderLoop on play should call update on all trackers and visualizers', async () => {
                initPlayer(mockResultData, jest.fn(), mockedDependencies);
                const audioPlayer = document.getElementById('audio-player');
                audioPlayer.currentTime = 1.23; // Set mock time

                // Start playback to kick off render loop
                audioPlayer.dispatchEvent(new Event('play'));
                const rAFCallback = global.requestAnimationFrame.mock.calls[0][0];
                await rAFCallback();
            ...
            test('should pause when data is not ready, then buffer and resume when data is ready', async () => {
                ...
                audioPlayer.dispatchEvent(new Event('play'));
                const rAFCallback = global.requestAnimationFrame.mock.calls[0][0];
                rAFCallback(); // Execute renderLoop
                await Promise.resolve(); // flush microtasks to reach await
                ...
                resolveEnsureData();
                await Promise.resolve(); // let the rest of renderLoop continue
            ...
            test('should NOT resume playback if user manually paused', async () => {
                ...
                // Initially we ARE playing so renderLoop enters the buffering state
                Object.defineProperty(audioPlayer, 'paused', { value: false, writable: true });
                const playPauseButton = document.getElementById('player-play-pause');
                playPauseButton.textContent = 'Pause'; // Initially looks like it's playing
                ...
                audioPlayer.dispatchEvent(new Event('play'));
                const rAFCallback = global.requestAnimationFrame.mock.calls[0][0];
                rAFCallback(); // Execute renderLoop up to the await
                await Promise.resolve(); // let microtasks run so ensureDataForTime is called and awaited

                // SIMULATE USER PAUSE DURING BUFFERING
                Object.defineProperty(audioPlayer, 'paused', { value: true, writable: true });
                playPauseButton.textContent = 'Play';

                // Resolve the buffer promise
                resolveEnsureData();
                await Promise.resolve(); // let the rest of renderLoop execute
        ...
        describe('BufferManager', () => {
            ...
            test('should load chunks incrementally, not all at once', async () => {
                const fetchSpy = mockHarmonicVisualizerInstance.streamAccessors.vocals._fetchChunk;
                // First interval fetches 1 chunk
                jest.advanceTimersByTime(500);
                expect(fetchSpy).toHaveBeenCalledTimes(1);

                // Second interval fetches next 1 chunk
                jest.advanceTimersByTime(500);
            ...
            test('should stop the interval when the queue is empty', async () => {
                ...
                // Worker fetches 1 per 500ms.
                // 10 intervals * 500ms = 5000ms
