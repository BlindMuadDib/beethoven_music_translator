...
describe('VolumeTracker (Overall RMS)', () => {
    ...
    beforeEach(() => {
        ...
        // Mock properties that jsdom doesn't compute layout for]
    ...
    test('constructor should initialize correctly and attach ResizeObserver', () => {
    ...
    test('resize should update canvas width/height to match offset values', () => {
        const tracker = new VolumeTracker('overall-volume-canvas');

        // Change offset values (simulating browser resize)
        Object.defineProperty(canvas, 'offsetWidth', { value: 200, configurable: true });
        Object.defineProperty(canvas, 'offsetHeight', { value: 500, configurable: true });

        tracker.resize();

        expect(tracker.canvas.width).toBe(200);
        expect(tracker.canvas.height).toBe(500);
    });
