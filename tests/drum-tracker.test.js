...
describe('DrumTracker', () => {
    ...
    describe('getYPosition', () => {
        ...
        test('should return correct base Y position for drum types', () => {
            // Snare Drum (Base 0.6). Pitch 0.
            // Weighted Pitch = 0. Factor = -0.5. Offset = -0.5 * 0.8 = -0.4.
            // Y = 0.6 - (-0.4) = 1.0
            let hit = { drum_category: 'snare', drum_type: 'open_band', spectral_centroid: 0, dominant_frequency: 0 };
            expect(tracker.getYPosition(hit)).toBeCloseTo(1, 0, 2);

            // Bass Drum (Base 0.9). Pitch 0. Y = 0.9 - (-0.4) = 1.3 -> 1.0 (Cap)
            hit = { drum_category: 'kick', drum_type: 'bass', spectral_centroid: 0, dominant_frequency: 0 };
            expect(tracker.getYPosition(hit)).toBeCloseTo(1.0, 2);

            // Hi-hat (yAxisMap: 0.25)
            // Base 0.25. Math.log10(1) = 0. Factor = (0 - 3.25)/1.5 = -2.16. Offset = -2.16 * 0.8 = -1.73
            // Y = 0.25 - (-1.73) = 1.98 -> 1.0
        ...
        test('should adjust Y position based on pitch (spectral_centroid and dominant_frequency)', () => {
            // Higher pitch decreases Y value (moves shape up)
            // Centroid 7000, Dom 8000. Weighted 7200.
            // log10(7200) = 3.857. Factor = (3.857 - 3.25) / 1.5 = 0.404. Offset = 0.404 * 0.8 = 0.323.
            // Snare Base 0.6. Y = 0.6 - 0.323 = 0.276
            ...
            expect(yPosHigh).toBeCloseTo(0.276, 2);

            // Lower pitch increases Y value
            // Centroid 100, Dom 50. Weighted = 90.
            // log10(90) = 1.954. Factor = (1.954 - 3.25) / 1.5 = -0.864. Offset = -0.864 * 0.8 = -0.691.
            // Snare Base 0.6. Y = 0.6 - (-0.691) = 1.29 -> 1.0 (Cap)
            ...
            expect(yPosLow).toBeCloseTo(1.0, 2);

            expect(yPosLow).toBeGreaterThan(yPosHigh);
        });

        test('should cap Y position between 0 and 1', () => {
            ...
            expect(tracker.getYPosition(hitVeryLowPitch)).toBeCloseTo(1.0, 2);
        });
    });
    ...
    describe('getSize', () => {
        ...
        test('should calculate size based on relative volume and decay', () => {
            const hit = { onset_time: 1.0, duration: 0.5, relative_volume: 0.8 };

            // At onset time, full size (since runningMaxVolume is now 0.8, ratio is 1.0)
            expect(tracker.getSize(hit, 1.0)).toBeCloseTo(100);

            // Halfway through duration, 50% decay for hits with duration > 0.15
            expect(tracker.getSize(hit, 1.25)).toBeCloseTo(100 * (1 - (0.25 / 0.5))); // 100 * 0.5 = 50

            // At end of duration, size should be 0
            expect(tracker.getSize(hit, 1.5)).toBeCloseTo(100 * (1 - (0.5 / 0.5))); // 100 * 0 = 0
        ...
        test('should not apply decay for hits with very short duration', () => {
            ...
            expect(tracker.getSize(shortHit, 1.05)).toBeCloseTo(100);
        });
    });
    ...
    describe('Tooltip Handling', () => {
        ...
        test('handleMouseMove should set hoveredHit using the accessor if mouse is over a visible hit', () => {
            ...
            // Calculate absolue pixel coordinates for the hit
            const verticalPixel = tracker.getYPosition(hit) * canvas.getBoundingClientRect().height;
            const horizontalPixel = tracker.config.staticXPosition * canvas.getBoundingClientRect().width;

            // Simulate mouse movement over the snare drum
            const mockEvent = {
                clientX: horizontalPixel,
                clientY: verticalPixel,
            };
