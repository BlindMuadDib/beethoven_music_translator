...
class TestTutorialGeneration(unittest.TestCase):
    ...
    def test_combine_generated_mock_audio_and_data(self):
        ...
        one stream of audio/data where each element follows the one before it
        with a short (2s) silent pause between each.
        This will be the final product for the front-end to process.
        """
        data, audio = _generate_tutorial.combine_generated_mock_audio_and_data()
        self.assertIsInstance(data, dict)
        self.assertIsInstance(audio, np.ndarray)
        self.assertIn('generate_harmonic_pitch_ramp_data_and_audio', data)
        # Check for significant length
        self.assertGreater(len(audio), 100 * _generate_tutorial.SAMPLE_RATE)
        # Check that a later segment's data is time-offset
        lyric_segment = data['generate_lyric_data_and_audio']
        self.assertGreater(lyric_segment['lyrics'][0]['line_start_time'], 150) # It's the last one, should be late
