...
class TestIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Allow environment override, default to KIND ports
        cls.base_url = os.environ.get("TEST_BASE_URL",
                                      "http://localhost:9000/api")
        cls.auth_base_url = os.environ.get("TEST_AUTH_URL",
    ...
    def get_authenticated_session(cls, email, password):
        ...
        try:
            ...
            if csrf_token:
                    admin_creds["csrf_token"] = csrf_token
            ...
            if csrf_token:
                access_request_data["csrf_token"] = csrf_token
            ...
            # Does the scraped URL start with /auth?
            if approve_url.startswith("/auth"):
                final_approve_url = f"{cls.base_url.replace('/api', '')}{approve_url}"
            else:
                final_approve_url = f"{cls.auth_base_url}{approve_url}"

            approve_resp = admin_session.post(final_approve_url,
                                              allow_redirects=True)
    ...
    def scrape_csrf_token(cls, html_text, form_action=None):
        """
        Finds a CSRF token in HTML text. Returns None if not found (e.g, testing mode)
        """
        if form_action:
        ...
        if not csrf_token_match:
            # Log specific warning but do not crash
            # !!! THIS SHOULD ONLY BE DONE IN TESTING !!!
            print("TESTING INFO: CSRF token not found in form. Assuming disabled (FLASK_ENV=testing).")
            return None
    ...
    def find_approve_url(cls, html_text, email):
        ...
        # 1. <td[^>]*> matches <td> tags with attributes (style, class, etc.)
        # 2. \s* handles newlines/spaces around the email
        pattern = (
            f'<tr>.*?'
            f'<td[^>]*>\\s*{re.escape(email)}\\s*</td>.*?'
            f'<form.*?action="([^"]+)">.*?</form>.*?</tr>'
        )
        ...
        print(f"DEBUG: Could not find row for {email}. HTML content:\n{html_text}")
        return None
    ...
    def setUp(self):
        ...
        self.audio_file_path = os.path.join(base_path,
                                            "data/audio/BloodCalcification-SkinDeep.wav")
        self.lyrics_file_path = os.path.join(base_path,
                                             "data/lyrics/BloodCalcification-SkinDeep.txt")
    ...
    def _run_translation_flow(self, target_url, files, expect_mapped_results):
        """
        Runs the entire translation flow for a successful translation then
        verifies the data structure against a known sample
        """
        ...
        self.assertEqual(job_status, 'finished',
                         f"Job did not finish. Final status: {job_status}")
        self.assertIsNotNone(final_job_result_data,
                             "Final job result data is None.")
        ...
        # Validate lyrics mapping portion if expected
        mapped_result = final_job_result_data["mapped_result"]
        if expect_mapped_results:
            ...
            for i, original_line in enumerate(original_lyrics_lines):
                ...
                self.assertEqual(mapped_line["line_text"],
                ...
                self.assertEqual(len(mapped_words_texts),
                                len(original_words_in_line),
            ...
            self.assertEqual(mapped_result, [],
                             "Expected empty mapped_result for instrumental")
            print("Verified: Instrumental track returned empty mapped_result list.")
        ...
        self.assertIsInstance(full_track_analysis["duration"], float)
        self.assertIsInstance(full_track_analysis["tempo"], float)
        self.assertIsInstance(full_track_analysis["rms_overall"], dict)
        ...
        if isinstance(drums_data, dict) and ("error" in drums_data or "info" in drums_data):
            ...
            self.assertIsInstance(hits, list, "Drum hits should be a list.")
            self.assertIsInstance(tempo, (float, int),
                                  "Drum tempo should be a float or int.")
            self.assertTrue(tempo >= 0, "Tempo must be non-negative.")
        ...
        # --- Validate MTR Download ---
        print("\nTesting .mtr download from live environment...")
        download_url = f"{self.base_url}/download-mtr/{job_id}"
        download_response = self.user_session.get(download_url, timeout=60)

        self.assertEqual(download_response.status_code, 200,
                         f"Failed to download .mtr file. Status: {download_response.status_code}")
        self.assertEqual(download_response.headers['Content-Type'], 'application/zip')

        # Verify Zip contents
        with zipfile.ZipFile(io.BytesIO(download_response.content)) as zf:
            file_list = zf.namelist()
            print(f"Downloaded .mtr contents: {file_list}")
            self.assertIn("result.json", file_list)
            self.assertIn("harmonic_static.json", file_list)
            # Check for streams
            streams  = [f for f in file_list if f.startswith("streams/")]
            self.assertTrue(len(streams) > 0,
                            "No stream files found in .mtr zip")

            # Check result.json version
            res_json = json.loads(zf.read("result.json"))
            self.assertEqual(res_json.get("version"), "0.1.4")
            print(".mtr file validation successful.")

            # Validate mapped_result presence and structure in .mtr
            self.assertIn("mapped_result", res_json)
            self.assertIsInstance(res_json["mapped_result"], list)

            if expect_mapped_results:
                self.assertTrue(len(res_json["mapped_result"]) > 0,
                                "Mapped result in .mtr is empty.")
                self.assertIn("line_text", res_json["mapped_result"][0])
            else:
                self.assertEqual(len(res_json["mapped_result"]), 0,
                                 "Expected empty mapped_result in .mtr for instrumental track.")

            # Validate drum_analysis presence and structure in .mtr
            self.assertIn("drum_analysis", res_json)
            self.assertIsInstance(res_json["drum_analysis"], dict)
            self.assertIn("hits", res_json["drum_analysis"])
            self.assertIn("tempo", res_json["drum_analysis"])
        ...
        cleanup_response = self.user_session.delete(cleanup_url, timeout=60)
    ...
    def test_translate_success_with_session(self):
        """
        Test standard translation with lyrics
        """
        if not self.lyrics_file: self.skipTest("Lyrics file missing")

        target_url = f"{self.base_url}/translate"
        files = {
            'audio': (os.path.basename(self.audio_file_path),
                      self.audio_file,
                      'audio/wav'),
            'lyrics': (os.path.basename(self.lyrics_file_path),
                       self.lyrics_file,
                       'text/plain')
        }

        self._run_translation_flow(target_url,
                                   files,
                                   expect_mapped_results=True)

    def test_translate_instrumental_success(self):
        """
        Test Instrumental Translation (No Lyrics)
        """
        print("\n--- Testing Instrumental Translation ---")
        target_url = f"{self.base_url}/translate"

        # Reset audio file pointer if reused or open fresh
        self.audio_file.seek(0)
        files = {
            'audio': (os.path.basename(self.audio_file_path),
                      self.audio_file,
                      'audio/wav')
            # No lyrics key provided
        }

        self._run_translation_flow(target_url,
                                   files,
                                   expect_mapped_results=False)
    ...
    def test_translate_unauthenticated(self):
        ...
        self.assertEqual(response_data["error"], "Access Denied. Please log in.")
    ...
    def test_library_integration(self):
        """Test retrieving the library metadata and fetching one of the MTR files."""
        print("\nTesting library integration...")
        library_url = f"{self.base_url}/library"
        
        # Test fetching the library
        response = self.user_session.get(library_url, timeout=20)
        self.assertEqual(response.status_code, 200)
        ...
        if len(data) > 0:
            ...
            self.assertIn("mtr_url", first_song)
            
            mtr_url = first_song["mtr_url"]
            base_host = self.base_url.rsplit('/api', 1)[0]
            if not mtr_url.startswith('/'):
                mtr_url = '/' + mtr_url
            full_mtr_url = f"{base_host}{mtr_url}"
            
            mtr_response = self.user_session.get(full_mtr_url, timeout=20)
            self.assertEqual(mtr_response.status_code, 200)
            self.assertNotEqual(mtr_response.headers.get('Content-Disposition', ''), 'attachment')
            print("Successfully retrieved library metadata and verified MTR is accessible but not as an attachment.")
        else:
            print("Library is empty. Skipping MTR check.")
