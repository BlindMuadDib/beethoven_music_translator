...
class TestMain(unittest.TestCase):
    ...
    def test_translate_missing_lyrics(self):
        ...
        lyrics are now optional so that instrumental music can be processed
        """
        self.mock_is_session_valid.return_value = True
        with open(self.test_audio_full_path, 'rb') as audio_file:
        ...
        self.mock_is_session_valid.assert_called_once_with("some-valid-session-token")
        self.mock_get_queue.assert_called_once() # Ensure queue was requested
        # Check if save was called once (for audio)
        ...
        expected_audio_path = f'/shared-data/audio/{self.test_job_id}_test_audio.wav'
        expected_task_args = (expected_audio_path, None,
                              f'{self.test_job_id}_test_audio.wav',
                              'test_audio.wav')
    ...
    def test_get_results_success_harmonic_analysis_had_error(self):
        """Tests results when harmonic analysis itself reported an error."""
    ...
    def test_background_translation_task_orchestration(
        ...
        expected_harmonic_result = {
            ...
            "streaming_urls": {
                "vocals": f"api/harmonic/stream/{self.test_job_id}_vocals.ndjson?stem_path=%2Ffake%2Fstems%2Fvocals.wav",
                "bass": f"api/harmonic/stream/{self.test_job_id}_bass.ndjson?stem_path=%2Ffake%2Fstems%2Fbass.wav"
            }
        }
    ...
    def test_download_mtr_success(self):
        """
        Tests the /api/download-mtr/<job_id> endpoint for a valid job.
        """
        self.mock_is_session_valid.return_value = True

        # Prepare mocked job result
        job_result = {
            "original_filename": "MySong.wav",
            "audio_url": "/api/files/jobid_MySong.wav",
            "harmonic_analysis": {
                "static_results_url": "api/results/file/jobid_harmonic.json",
                "streaming_urls": {
                    "vocals": "api/harmonic/stream/jobid_vocals.ndjson?stem_path=...",
                    "bass": "api/harmonic/stream/jobid_bass.ndjson?stem_path=..."
                }
            },
            "mapped_result": [{"line_text": "Test lyrics",
                               "line_start_time": 0.0,
                               "line_end_time": 5.0}],
            "drum_analysis": {"hits": [{"onset_time": 1.0, "drum_type": "snare"}],
                              "tempo": 120}
        }

        self.mock_job.is_finished = True
        self.mock_job.result = job_result

        # Mock file system reads using patch on 'builtins.open'
        # Needs to handle multiple file reads:
        # 1. harmonic_static.json
        # 2. vocals.ndjson
        # 3. bass.ndjson

        mock_static_content = b'{"full_track_analysis": {}}'
        mock_stream_content = b'{"time": 0.1}'

        with patch('builtins.open', create=True) as mock_open, \
             patch('os.path.exists', return_value=True):

            # Setup side_effect to return different file contents based on call
            mock_file = MagicMock()
            mock_file.__enter__.return_value.read.side_effect = [
                mock_static_content,    # First read (static)
                mock_stream_content,    # Second read (vocals)
                mock_stream_content     # Third read (bass)
            ]
            mock_open.return_value = mock_file

            with self.client as c:
                c.set_cookie('session', 'valid-cookie')
                response = c.get(f'/api/download-mtr/{self.test_job_id}')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers['Content-Type'], 'application/zip')
        self.assertEqual(
            response.headers['Content-Disposition'],
            'attachment; filename=MySong.mtr'
        )
        ...
        with zipfile.ZipFile(io.BytesIO(response.data), 'r') as zf:
            ...
            self.assertIn('harmonic_static.json', files)
            self.assertIn('streams/vocals.ndjson', files)
            self.assertIn('streams/bass.ndjson', files)

            # Check result.json modifications and ensure components are present
            result_json = json.loads(zf.read('result.json'))

            # 1. Check versioning
            self.assertEqual(result_json['version'], "0.1.4")

            # 2. Check Audio Exclusion
            self.assertIsNone(result_json['audio_url'])

            # 3. Check Harmonic modifications
            self.assertEqual(
                result_json['harmonic_analysis']['static_results_url'],
                "harmonic_static.json"
            )
            self.assertEqual(
                result_json['harmonic_analysis']['streaming_urls']['vocals'],
                "streams/vocals.ndjson"
            )

            # 4. Check existence of lyrics and drums
            self.assertEqual(len(result_json['mapped_result']), 1)
            self.assertEqual(result_json['mapped_result'][0]['line_text'],
                             "Test lyrics")
            self.assertEqual(result_json['drum_analysis']['tempo'], 120)
            self.assertEqual(len(result_json['drum_analysis']['hits']), 1)

    def test_download_mtr_job_not_finished(self):
        """
        Tests .mtr download when job is not finished.
        """
        self.mock_is_session_valid.return_value = True
        self.mock_job.is_finished = False
        ...
        self.assertEqual(response.status_code, 404)
        self.assertIn("Job not finished", json.loads(response.data)['error'])

    def test_get_library_api(self):
        """Tests the logic of the /api/library endpoint to return CC songs sorted."""
        mock_metadata = [
            {"artist": "Zebra", "title": "B", "filename_prefix": "Zebra-B", "license": "Creative Commons"},
            {"artist": "Aardvark", "title": "A", "filename_prefix": "Aardvark-A", "license": "Creative Commons"},
            {"artist": "Zebra", "title": "A", "filename_prefix": "Zebra-A", "license": "Creative Commons"},
            {"artist": "Evil", "title": "Song", "filename_prefix": "Evil-Song", "license": "Copyright"}
        ]
        mock_file_content = json.dumps(mock_metadata)
        ...
        self.assertEqual(data[0]['artist'], 'Aardvark')
        self.assertEqual(data[0]['title'], 'A')
        self.assertEqual(data[1]['artist'], 'Zebra')
        self.assertEqual(data[1]['title'], 'A')
        self.assertEqual(data[2]['artist'], 'Zebra')
        self.assertEqual(data[2]['title'], 'B')
