...
describe('API Module', () => {
    ...
    describe('pollJobStatus', () => {
        ...
        test('should poll until "finished", see a results_url, fetch it, and return merged data', async () => {
            ...
            // Check that the MockAccessor was called correctly
            ...
            // Verify that ensureDataForTime(0) was called on the harmonic
            // accessor.
            const accessorInstances = MockAccessor.mock.results.map(r => r.value);

            // First two should be harmonics (vocals, bass)
            expect(accessorInstances[0].ensureDataForTime).toHaveBeenCalledWith(0);
            expect(accessorInstances[1].ensureDataForTime).toHaveBeenCalledWith(0);
    ...
    describe('triggerMtrDownload', () => {
        ...
        test('should set window.location.href to the download URL', () => {
            API.triggerMtrDownload('job-123');
            expect(window.location.href).toBe('/api/download-mtr/job-123');
        });

        test('should log error if no job ID is provided', () => {
            API.triggerMtrDownload(null);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Cannot download MTR: Missing Job ID.");
            expect(window.location.href).toBe('');
        });
    });

    describe('fetchLibrary', () => {
        test('should fetch and return the library data as JSON', async () => {
            const mockLibraryData = [
                { artist: 'Artist A', title: 'Song', mtr_url: '/api/file.mtr', audio_url: '/api/file.wav' }
            ];
            fetch.mockResolvedValueOnce({
                ok: true,
        ...
        test('should throw an error on failed fetch', async () => {
            ...
            await expect(API.fetchLibrary()).rejects.toThrow('Failed to load library metadata');
        });
    });
});
