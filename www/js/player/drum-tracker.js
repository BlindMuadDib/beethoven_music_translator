...
export class DrumTracker {
    ...
    constructor(canvas, drumAnalysis) {
        ...
        this.config = {
            ...
            maxSize: 150, // Max size of a shape
            yAxisPitchRange: 0.8, // How much pitch can affect Y-pos
    ...
    getYPosition(hit) {
        ...
        // Normalize the pitch value logarithmically since pitch perception is non-linear.
        // We'll map a practical range of roughly 316Hz (log10=2.5) to 10000Hz (log10=4.0).
        // A lower pitch pushes the shape down (+Y), higher pitch pushes it up (-Y).
        // Normalize to roughly -0.5 to 0.5
        const logPitch = Math.log10(Math.max(weightedPitch, 1));
        const pitchFactor = (logPitch - 3.25) / 1.5;
    ...
    getSize(hit, currentTime) {
        ...
        // Dynamically track the running maximum volume over time to normalize hit sizes
        if (!this.runningMaxVolume || hit.relative_volume > this.runningMaxVolume) {
            // Ensure we have a small minimum floor
            this.runningMaxVolume = Math.max(0.01, hit.relative_volume);
        }

        // We use the runningMaxVolume to bring the current hit.relative_volume up proportionally
        const normalizedVolume = Math.min(1.0, hit.relative_volume / this.runningMaxVolume);

        return this.config.maxSize * normalizedVolume * decayFactor;
    }
