...
export class HarmonicVisualizer {
    ...
    constructor(canvas, harmonicData) {
        ...
        this.config = {
            ...
            baseSaturation: 80, // %
            baseLightness: 60, // % Use 60 to bring the brightness of the blobs up
            minBlobWidth: 5,
        ...
        // Defer initial draw until next animation frame
        // This ensures the browser has applied layout (flexbox) updates from
        // ui.updateUIVisibility so offsets are correct
     ...
     */
    resize() {
        // If the container is hidden, offsetWidth will be 0. Do nothing.
        if (this.canvas.offsetWidth === 0 || this.canvas.offsetHeight === 0) return;
     ...
     */
    update(currentTime) {
        // If the canvas has no size (hidden), don't waste cycles
        if (this.canvas.width === 0 || this.canvas.height === 0) return;

        this.currentTime = currentTime;
        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);

        // Safety check: If axes are missing (e.g. initial render failed), retry drawing
        if (this.axisContainer && this.axisContainer.childElementCount === 0) this.drawFrequencyAxis();
        if (this.chromaKeyContainer && this.chromaKeyContainer.childElementCount === 0) this.drawChromaKey();
    ...
    drawInstrumentColumn(x, width, instrumentName, stemData) {
        ...
        const lightness = this.mapValueToLightness(spectralCentroid, 10000); // 10000Hz max for centroid assumption
        ...
        const hasAudio = rms !== undefined && rms > 0.001;

        // Draw the main blob using spectrogram and spectral data
        if (hasAudio && frequencies && frequencies.length > 1) {
            if (f0 && f0 > 0) {
                this.drawBlob(
                    centerX,
                    blobWidth,
    ...
    /**
     * Maps a spectral centroid value to a lightness in HSL space.
     * @param {number} value - The spectral centroid value.
     * @param {number} max - The max value for normalization.
     * @returns {number} - The lightness value (%).
     */
    mapValueToLightness(value, max) {
        // High centroid (bright sound) -> High lightness
        // Low centroid (dark sound) -> Low lightness
        // Should be between 30 and 80
        const normalized = Math.min(1, Math.max(0, value / max));
        return this.config.baseLightness * normalized + 30; // Ensure it's not completely black
    }
    ...
    drawBlob(x, radius, color, f0, spectralCentroid, spectralBandwidth, spectralFlatness, spectrogram, frequencies, mfccs) {
        ...
        // Find min and max y based on lowest and highest frequencies in the bin
        const highestFreq = frequencies[frequencies.length - 1];
        const lowestFreq = frequencies[0];
        const normalizedMinY = this.mapValueToLogNormalizedY(highestFreq);
        const minY = this.canvas.height - (normalizedMinY * this.canvas.height);
        const normalizedMaxY = this.mapValueToLogNormalizedY(lowestFreq);
        const maxY = this.canvas.height - (normalizedMaxY * this.canvas.height);

        this.drawMfccTexture(minY, maxY, minX, maxX, mfccs);
     ...
     * @param {number} minY - The top bound of the texture.
     * @param {number} maxY - The bottom bound of the texture.
     * @param {number} minX - The left bound of the texture.
     * @param {number} maxX - The right bound of the texture.
     ...
     */
    drawMfccTexture(minY, maxY, minX, maxX, mfccs, frequencies = []) {
        ...
        // Fixed cell size based on global canvas for persistent mosaic
        const cellSize = Math.max(40, this.canvas.height / 15);

        // Calculate visible grid rows and columns based on absolute global coordinates
        ...
        // Simple pseudo-random hash function to pick deterministic MFCC values based on grid position
        const getHash = (q, r) => {
            let h = Math.abs(Math.sin(q * 12.9898 + r * 78.233) * 43758.5453123);
            return Math.floor(h * 1000) % mfccLen;
        };

        for (let i = startRow; i < endRow; i++) { // Grid row (global)
            for (let j = startCol; j < endCol; j++) { // Grid column (global)
                // Calculate center of the current grid cell globally
                const cellX = (j * cellSize) + (cellSize / 2);
                const cellY = (i * cellSize) + (cellSize / 2);

                // Hash i and j to predictably grab an MFCC index
                const mfccIndex = getHash(i, j);

                // Use a smaller baseline radius so shapes overlap slightly but don't become overwhelmingly large
                const shapeRadius = cellSize * (0.4 + Math.abs(mfccs[mfccIndex] / maxMfcc) * 0.4);
    ...
    drawChromaHoops(x, y, columnWidth, chromaStft) {
        ...
        // 90% of the maximum value in this frame. Adjust as needed
        const relativeThreshold = maxChromaValue * 0.90;
        ...
        for (let i = 0; i < 12; i++) {
            ...
            // Inverted logic so higher notes (higher i) are drawn higher (lower Y)
