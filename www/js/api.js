...
const TUTORIAL_DATA_URL = '/data/tutorial/data.json';
const TUTORIAL_AUDIO_URL = '/data/tutorial/audio.wav';
...
/**
 * Fetches the static assets required for the tutorial mode.
 * @returns {Promise<object>} The tutorial data object, including the audio URL.
 */
export async function fetchTutorialData() {
    const [dataRes, audioRes] = await Promise.all([
        fetch(TUTORIAL_DATA_URL),
        fetch(TUTORIAL_AUDIO_URL)
    ]);

    if (!dataRes.ok || !audioRes.ok) {
        throw new Error("Failed to load tutorial assets.");
    }

    const data = await dataRes.json();
    // The audio_url in the json might be generic, override it with the known asset
    data.audio_url = TUTORIAL_AUDIO_URL;
    return data;
}
...
async function prepareFinalData(initialResult) {
    ...
    if (staticData.stem_analyses) {
        for (const instrument in staticData.stem_analyses) {
            ...
            if (stemStaticData && harmonicInfo.streaming_urls?.[instrument]) {
                ...
                // Push the accessor OBJECT, do not call the method yet
                accessorsToLoad.push(accessor);
            }
        }
    }
    ...
    console.log(`Waiting for initial data chunks for ${accessorsToLoad.length} instruments...`);

    // Load sequentially to avoid 503 Server Overload
    for (const accessor of accessorsToLoad) {
        try {
            await accessor.ensureDataForTime(0);
        } catch (e) {
            console.warn("Failed to pre-load data chunk, will retry in player.", e)
        }
    }
...
/** Triggers the browser download of the .mtr file for the given job.
 * @param {string} jobId - The ID of the job to download.
 */
export function triggerMtrDownload(jobId) {
    if (!jobId) {
        console.error("Cannot download MTR: Missing Job ID.");
        return;
    }
    // Trigger a direct browser download
    window.location.href = `/api/download-mtr/${jobId}`;
}

/**
 * Fetches the library metadata of available songs.
 * @returns {Promise<Array>} A list of available songs.
 */
